...
BOOL CProSliderDlg::OnInitDialog()
{
	...
	// debug screen init
	CString lpszString;
	lpszString.Format(_T("%d"),this->m_TimerValue);
	m_EditTimer.SetWindowText(lpszString);
	lpszString.Format(_T("%d"),this->m_TimerValue1);
...
void CProSliderDlg::OnEnChangeEdit()
{
	...
	this->m_TimerValue = _wtoi(rString);
}
...
void CProSliderDlg::OnEnChangeEdit2()
{
	...
	this->m_TimerValue1 = _wtoi(rString);	
}

void CProSliderDlg::OnBnBorders()
{
	m_ProSlider._IsEnabled()?m_ProSlider._EnableBorders(FALSE):m_ProSlider._EnableBorders(TRUE);
	m_ProSlider._IsEnabled()?m_bnBorders.SetWindowText(_T("No Borders")):m_bnBorders.SetWindowText(_T("Borders"));
...
void CProSliderDlg::OnBnFreeze()
{
	...
	ret?m_bnFreeze.SetWindowText(_T("Freeze")):m_bnFreeze.SetWindowText(_T("Defreeze"));
}
